...
description: 매일 아침 8시, 건축·현상설계에 쓸 수 있는 AI 도구·오픈소스·논문을 자동으로 모아 정리합니다. 단순 신기한 데모가 아니라, URL 그대로 가져다 바로 작업 시작할 수 있는 것만 추립니다.
stats:
  - num: "매일"
    lbl: "Daily Update"
  - num: "6"
...
기준은 단순합니다 — **"이 URL을 AI 코딩 도구에 박으면 바로 작업이 시작되는가?"** 단순히 멋진 데모나 마케팅 글은 거릅니다. 클론받아서 자기 프로젝트에 통합 가능한 것 위주.
...
> 예시 — IFC(BIM 표준 파일) 파서 라이브러리 URL을 박고 한 줄:
> "이 라이브러리로 내 프로젝트 BIM 파일에서 모든 방의 면적·인접 관계 자동 추출하는 스크립트 짜줘."

→ 30분 안에 자기 사무실 워크플로우에 끼워넣을 수 있는 도구가 만들어집니다.

## 누가 보면 좋을까

- **건축가 · 현상설계 디자이너** — 컨셉 단계 가속, 법규·시공성 자동검토 도구 발굴
- **건축 · 도시 학생** — 자기 워크플로우에 AI 끼워넣고 싶은 사람
