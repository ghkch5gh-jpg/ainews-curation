...
- 건축 온톨로지: **연구 흐름 + 본인 프로젝트에 통합할 인프라** 둘 다
- 빌더 마인드 — 단순 데모 ❌ / 파이프라인에 끼워 넣을 수 있는 것 ✅

## 1. 절대 필터 — "Claude Code에 URL 입력하면 작업이 시작되는 자료"
...
- ★★★ — 사용자 도메인 직격. 컨셉 또는 법규/온톨로지 단계에 즉시 통합할 수 있음. 코어 섹션.
...
**기본 자세 (2026-05-11 갱신):** 친구가 카톡으로 설명하듯. 코드 한 줄도 안 짜본 건축가가 읽어도 *이게 뭐 하는 거고 / 나한테 어떤 의미인지* 한 번에 이해되어야 함. 못 알아들어서 사용자가 도구를 안 쓰게 되면 큐레이션 실패.

### 핵심 원칙

1. **"이게 뭐냐"를 한 마디로 — 비유·비교 우선.**
   - "Three.js로 한 단계 위 인터랙티브 뷰어" ❌ → "PPT보다 한 단계 위, 발주처가 마우스로 돌려보는 3D" ✅
   - 사용자가 이미 아는 것(PPT·DWG·ChatGPT·Revit)에 빗대 설명. 모르는 단어로 모르는 단어 설명 X.

2. **"왜 의미 있냐"는 구체적 시나리오 — 누구·언제·뭐 하다가.**
   - "법규 자동 검토에 활용" ❌ → "심사위원이 모델 클릭하면 면적·재료 정보 옆에 뜸. 발표할 때 짧은 영상 대신 진짜 모델 띄움" ✅
   - "컨셉 단계 가속" 같은 추상어 금지. **"이거 있으면 너는 ___할 수 있어"** 형태로.

3. **CS 약어는 처음 등장 시 풀이 — 빠짐없이.**
   - 이전에는 "API·SDK·repo는 그대로"였지만 변경. 건축가는 모름. 처음 등장 시 풀이.
   - 예: "repo(코드 저장소)", "library(미리 만들어진 도구 묶음)", "SDK(개발자 키트 — 이걸 갖다 쓰면 직접 안 만들어도 됨)", "API(외부 도구 호출 통로)"
   - 예외: 한 항목 안에서 두 번째부턴 생략 OK. 다른 항목에서 다시 풀이.

4. **AI/ML 용어 — 풀이 + 짧은 비유.**
   - "LLM(언어 모델 AI — ChatGPT 같은 거)"
   - "임베딩(비슷한 것끼리 숫자로 가깝게 매김 — 검색 잘하게 하는 기초)"
   - "NeRF(사진 여러 장 → 3D 자동 재구성)"
   - "diffusion(이미지·영상 생성 AI 방식)"
   - "RAG(자료 미리 주고 답하게 하는 방식)"

5. **건축 약어는 그대로 OK** — IFC·BIM·RVT·DWG·DGN·glTF·OBJ·FBX·MEP·HVAC·CAD·GH(그래스호퍼). 풀이 불필요.

6. **금지 단어/표현:**
   - "워크플로우 가속", "파이프라인 통합", "에코시스템", "스택" 같은 일반 추상어
   - WebAssembly·transformer·feedforward·topology(CS) 같은 무거운 CS 용어 — 모두 의역 ("엄청 빠른 속도", "AI", "한 번에", "구조"로)
   - 슬랭 ("박다", "꽂다") 절대 금지 — 공개 콘텐츠

### 항목 작성 형식

**🔥 GitHub 인기 항목 (도메인 무관, 5개)** — meta line 다음 줄에 짧은 설명 + 건축 활용:
```markdown
### N. [도구 이름](URL)
GitHub Trending | XXk ⭐ | +XX 이번 주

이 도구가 뭐 하는지 한 문장 (비유 포함, 굵게 강조 표시 자제 — meta line 위 첫 단락은 그대로 텍스트로).

**건축 활용:** 한 줄 — 직접 무관/시도 가능 등 솔직히.
```

**★★★ Core / ★★ Sub 항목** — meta line 다음 줄부터 라벨 단락:
```markdown
### [도구 이름](URL)
★등급 | 소스 | XXk ⭐ | 마지막 갱신 (활발/일반)

**한 줄로:** 친구가 1초 만에 이해하는 설명. 비유 포함.

**왜 의미 있냐:**
- 시나리오 1 (구체적 — 누구가 언제 뭐 하다가)
- 시나리오 2
...
git clone <URL>
\`\`\`
→ "자연어 지시 한 줄 — 자기 일에 끼워 넣는 형태로"

**왜 이번 주:** 1줄.
```

**핵심 — 첫 단락은 무조건 meta line.** h3 직후 바로 본문이 오면 CSS가 자동으로 meta-line 스타일(작은 글씨·연한 색·하단 구분선)을 입혀서 *본문이 메타로 보임*. h3 다음에 **반드시 한 줄짜리 meta line**(소스·별·갱신일 등) 먼저, 그 다음에 빈 줄, 그 다음 본문.

### 분량·호흡

- "한 줄로" 항목은 **진짜 한 문장**. 두 문장 이상이면 잘림.
- "왜 의미 있냐" 불릿 2~3개. 각 1~2줄.
- 같은 개념을 두 단어로 부르지 말 것 (예: "임베딩 = 벡터 표현" — 둘 중 하나만)
- 풀이 추가한다고 줄 늘리지 말 것 — 괄호 안에 한 호흡으로

### 톤 점검 셀프 질문 (작성 후)

각 항목 작성 후 다음 자문:
1. 이 한 줄을 *코드 한 줄도 안 짜본 건축가*가 읽고 머리에 그림이 그려지나?
2. "이거 있으면 너는 ___" 빈칸이 구체적인 행위인가? (추상어 X)
3. 모르는 단어가 또 다른 모르는 단어로 설명되어 있지 않은가?
...
- **현재 운영:** 매일 08:00 KST (cron: `0 8 * * *` 로컬 타임)
